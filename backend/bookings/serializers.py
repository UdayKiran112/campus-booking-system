...
class BookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Booking
        fields = [
            'id', 'user', 'user_details', 'venue', 'venue_details',
            'title', 'description', 'booking_date', 'start_time', 'end_time',
            'duration_hours', 'priority', 'priority_display', 'status', 'status_display',
            'approved_by', 'approved_by_details', 'approval_date', 'rejection_reason',
            'payment_required', 'payment_amount', 'payment_type', 'payment_status',
            'payment_id', 'security_deposit', 'deposit_refunded',
            'checked_in', 'check_in_time', 'checked_out', 'check_out_time',
            'booking_reference', 'invoice_generated', 'invoice_sent',
            'is_past', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'duration_hours', 'approved_by', 'approval_date',
            'payment_id', 'payment_date', 'deposit_refunded', 'refund_date',
...
class BookingCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Booking
        fields = [
            'venue', 'title', 'description', 'booking_date',
    ...
    def validate(self, data):
        ...
        if data['start_time'] < venue.available_from:
            raise serializers.ValidationError(
                f"Venue opens at {venue.available_from}"
            )
        if data['end_time'] > venue.available_to:
            raise serializers.ValidationError(
                f"Venue closes at {venue.available_to}"
            )
    ...
    def create(self, validated_data):
        ...
        validated_data['payment_required'] = pricing['base_price'] > 0 or pricing['security_deposit'] > 0
...
class ConflictResolutionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'original_booking', 'original_booking_details',
            'new_booking', 'new_booking_details', 'conflict_detected_at',
            'resolution_status', 'alternative_slots_offered',
            'user_response_deadline', 'resolved_at'
        ]
...
class BookingModificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'booking', 'modified_by', 'modified_by_details',
            'old_date', 'new_date', 'old_start_time', 'new_start_time',
            'old_end_time', 'new_end_time', 'old_status', 'new_status',
            'reason', 'modified_at'
        ]
...
class InvoiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Invoice
        fields = [
            'id', 'booking', 'booking_details', 'invoice_number',
            'invoice_date', 'venue_charge', 'security_deposit',
            'tax_amount', 'total_amount', 'pdf_file'
        ]
