...
from .serializers import (
    ...
    ConflictResolutionSerializer, InvoiceSerializer,
    SlotAvailabilitySerializer, AlternativeSlotSerializer
)
    ...
    def get_queryset(self):
        ...
        if not user.is_authority:
            ...
            controlled_venues = Venue.objects.filter(
                Q(controlling_authority=user) |
                Q(department=user.department)
            )
            queryset = queryset.filter(
                Q(user=user) | Q(venue__in=controlled_venues)
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not availability_response['is_available']:
            ...
            return Response({
                'available': False,
                'conflict': False,
                'message': availability_response.get('reason', 'Slot is not available')
...
def get_alternative_slots_view(request, venue_id):
    ...
    for day_offset in range(7):
        ...
        while True:
            ...
            slot_end_dt = datetime.combine(check_date, current_time) + timedelta(hours=duration_hours)
