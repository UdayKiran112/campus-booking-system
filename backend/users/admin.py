...
class UserAdmin(BaseUserAdmin):
    ...
    fieldsets = BaseUserAdmin.fieldsets + (
        ('Role & Department', {
            'fields': ('role', 'department', 'verified')
        }),
        ('Additional Info', {
            'fields': ('phone_number', 'employee_id', 'student_id', 'organization_name')
        }),
    )
...
class UserProfileAdmin(admin.ModelAdmin):
    list_display = ['user', 'no_show_count', 'total_bookings', 'successful_bookings', 'reliability_score']
