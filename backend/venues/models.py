...
class Venue(models.Model):
    ...
    controlling_authority = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="controlled_venues"
    )
    ...
    def can_user_approve(self, user):
        ...
        if (
            ...
            and self.ownership == self.OwnershipType.ADMINISTRATION
        ):
            return True

        if (
            user.role == User.UserRole.DEPARTMENT_HEAD
            and self.ownership == self.OwnershipType.DEPARTMENT
        ):
            return user.department == self.department

        if (
            user.role == User.UserRole.SCHOOL_OFFICE
            and self.ownership == self.OwnershipType.SCHOOL_OFFICE
        ):
            return user.department == self.department

        if (
            user.role == User.UserRole.WARDEN
            and self.ownership == self.OwnershipType.WARDEN_OFFICE
        ):
            return True
    ...
    def get_pricing_for_user(self, user):
        ...
        if user.role == User.UserRole.FACULTY:
            ...
            return {"base_price": 0, "security_deposit": 0, "payment_type": "FREE"}
...
class VenueAvailability(models.Model):
    ...
    venue = models.ForeignKey(
        Venue, on_delete=models.CASCADE, related_name="special_availability"
    )
...
class VenueImage(models.Model):
    ...
    venue = models.ForeignKey(
        Venue, on_delete=models.CASCADE, related_name="venue_images"
    )
