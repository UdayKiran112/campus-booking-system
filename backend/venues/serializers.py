...
class VenueSerializer(serializers.ModelSerializer):
    controlling_authority_details = UserSerializer(
        source="controlling_authority", read_only=True
    )
    ...
    venue_type_display = serializers.CharField(
        source="get_venue_type_display", read_only=True
    )
...
class VenueCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data["ownership"] == Venue.OwnershipType.DEPARTMENT and not data.get(
            "department"
        ):
            raise serializers.ValidationError(
                "Department is required for department-controlled venues"
            )
...
class VenueSearchSerializer(serializers.Serializer):
    ...
    category = serializers.ChoiceField(
        choices=Venue.VenueCategory.choices, required=False
    )
    venue_type = serializers.ChoiceField(
        choices=Venue.VenueType.choices, required=False
    )
    ownership = serializers.ChoiceField(
        choices=Venue.OwnershipType.choices, required=False
    )
